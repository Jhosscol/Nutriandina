...
export default function CitasScreen({ navigation }) {
  ...
  const loadNutritionists = async (token) => {
    try {
      // 🔥 CAMBIO: Ruta corregida
      const url = `${API_URL}/appointments/nutritionists`;
      ...
      if (response.ok) {
        ...
        Alert.alert('Error', data.message || 'Error al cargar');
      }
    } catch (error) {
  ...
  const loadAvailability = async (nutritionistId, date) => {
    try {
      const token = await getToken();
      // 🔥 CAMBIO: Ruta corregida
      const url = `${API_URL}/appointments/nutritionists/${nutritionistId}/availability?date=${date}`;
      
      console.log('🔄 Cargando disponibilidad:', url);
      
      const response = await fetch(url, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      
      console.log('📊 Status availability:', response.status);
